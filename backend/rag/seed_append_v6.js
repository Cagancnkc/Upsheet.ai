...
const NEW_CATEGORIES = ['heatmap', 'compare', 'forecast', 'anomaly_detection', 'sentiment_analysis', 'classify', 'extract'];

async function seedAppendV6() {
  const NEW_EXAMPLES = EXCEL_DATASET.filter(item => NEW_CATEGORIES.includes(item.category));
  console.log('V6 yeni kategori örnek sayısı:', NEW_EXAMPLES.length);
  ...
  console.log(`\n✓ V6 ekleme tamamlandı. Eklenen: ${loaded}/${toAdd.length}`);
  console.log('✓ Önceki veriler korundu.');
}

seedAppendV6().catch(console.error);
